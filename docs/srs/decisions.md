...
## ADR-008 — Companion app is a Web Bluetooth client over a custom GATT config service

- **Status:** Accepted
- **Date:** 2026-06-08
- **Context:** We want a user-facing app to tune motion parameters, view live telemetry, and trigger calibration without re-flashing. Three axes of choice: (a) platform — native iOS/Android vs. web; (b) transport — reuse the HID link vs. a separate GATT service; (c) pairing — app-driven vs. OS-driven. Hard facts: the glove is an OS-paired BLE HID mouse (ADR-004) and `TC-NFR-HID-001` depends on it staying a plain mouse; Web Bluetooth cannot access HID (blocklisted) and is unavailable on iOS/Safari; the primary demo host is a Windows laptop; "companion mobile app" was previously out of scope.
- **Decision:** Build a **web** companion app using **Web Bluetooth** (Chrome/Edge desktop + Android). It connects to a **new custom GATT config/telemetry service** that is separate from the HID profile and shares the existing NimBLE server. HID pairing/bonding remains **OS-owned**; the app performs only a GATT connection to the config service. Native mobile is not pursued. The GATT contract is specified in `docs/plans/10-companion-app.md` (E15, FR-011).
- **Consequences:** Positive — zero-install, runs on the primary Windows host, decoupled from the HID path (blast radius is a separate service + a `companion/` folder), and a mock client lets the UI demo without firmware. Negative — no iOS support (Web Bluetooth limitation, accepted); a new firmware GATT service + NVS bridge is required (overlaps E11/E12); telemetry frame (24 B) needs MTU negotiation or splitting. Trade-off accepted for a class-project demo.
